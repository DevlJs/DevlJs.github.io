/*!
<form id="reqFrom" subUrl="http://121.43.233.53:1898/api/VVb0FKGpGHBL5idU">
  <input type="text" name="UserName" required-rr="不能为空" req-max="10" req-min="2">
  <input type="text" name="Tel" required-rr="电话不能为空" reqMessage="电话格式不对" reqReg="^1\d{10}$">
  <input type="text" required-rr="邮箱不能为空" reqMessage="邮箱格式不对" reqReg="^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+">

  <input type="button" value="验证" onclick="validate();"/>

  <div id="popSucess" style="display:none;">
    提交成功
  </div>
</form>
...
(function(factory) {
  ...
  $.fn.serializeSubmie = function(options) {
    ...
    if (valResult) {
      ...
      $.post($(this).attr("subUrl"), serializedObject, function(res) {
        if (res.success) {
          alert("信息提交成功!");
        } else {
          alert("网络繁忙，请稍后再试!");
  ...
  $.fn.validate = function() {
    var _self = $(this);
    //为空判断
    eachElement('required-rr', _self, function() {
      eachElement('req-min', _self, function() {
        eachElement('req-max', _self, function() {
          eachElement('reqReg', _self)
        });
      });
    });
    return flag;
  }
  function eachElement(type, obj, next) {
    flag = true;

    var element = obj.find("[" + type + "]");
    for (var i = 0, max = element.length; i < max; i++) {
      var val = $(element[i]).val();
      switch (type) {
        case "required-rr":
          if (val.length == 0) {
            alert($(element[i]).attr("" + type + ""));
            flag = false;
            return;
          }
          break;
        case "req-min":
          if (val.length < parseInt($(element[i]).attr("" + type + ""))) {
            alert("最小不能少于" + $(element[i]).attr("" + type + "") + "位");
            flag = false;
            return;
          }
          break;
        case "req-max":
          if (val.length > parseInt($(element[i]).attr("" + type + ""))) {
            alert("最小不能大于" + $(element[i]).attr("" + type + "") + "位");
            flag = false;
            return;
          }
          break;
        case "reqReg":
          var telReg = $(element[i]).attr("" + type + "");
          var regExp = new RegExp(telReg);
          if (!regExp.test(val)) {
            alert($(element[i]).attr("reqMessage"));
            flag = false;
            return;
          }
          break;
